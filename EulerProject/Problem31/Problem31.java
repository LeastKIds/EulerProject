class Problem31
{
  ...
  public static int switchLoop(final int MAKE_POUND, int combination, int count, int digit)
  {
    ...
    switch(combination)
    {
      case 2: count+=MAX_DIGIT+1;
              //System.out.println(count);
              break;
      case 5: for(int i=digit_count_5; i<=MAX_DIGIT; i++)
                count=switchLoop(MAKE_POUND-(i*5),2,count,digit);
              //count+=1;
              //System.out.println(count);
              break;
      case 10: for(int i=digit_count_10; i<=MAX_DIGIT; i++)
                count=switchLoop(MAKE_POUND-(i*10),5,count,digit);
              //count+=1;
              break;
      case 20: for(int i=digit_count_20; i<=MAX_DIGIT; i++)
                count=switchLoop(MAKE_POUND-(i*20),10,count,digit);
              //count+=1;
              break;
      case 50: for(int i=digit_count_50; i<=MAX_DIGIT; i++)
                count=switchLoop(MAKE_POUND-(i*50),20,count,digit);
              //count+=1;
              break;
      case 100: for(int i=digit_count_100; i<=MAX_DIGIT; i++)
                  count=switchLoop(MAKE_POUND-(i*100),50,count,digit);
                //count+=1;
                break;
    }
    return count;
